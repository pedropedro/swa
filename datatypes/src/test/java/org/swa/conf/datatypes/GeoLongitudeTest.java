...
public class GeoLongitudeTest {
	...
	public void decimalTest() {
		...
		System.out.println("Crunched decimals: " + count + " in " + ((System.currentTimeMillis() - start) / 1000) + " [s]");
	}
	...
	static class ForkComparator extends RecursiveTask<Integer> {
			...
			for (int sign = sign_from; sign <= sign_to; sign++)
				for (int decimal = decimal_from; decimal <= decimal_to; decimal++)
					for (int fraction = fraction_from; fraction <= fraction_to; fraction++) {
						final String s = (sign == 0 ? "-" : "") + String.valueOf(decimal) + "." + String.format("%06d", fraction);
						counter++;
		...
		protected Integer compute() {
			...
			if (sign_from != sign_to) {
				...
				final ForkComparator f1 = new ForkComparator(sign_from, sign_from + splitPoint, decimal_from, decimal_to,
						fraction_from, fraction_to);
				f1.fork();

				final ForkComparator f2 = new ForkComparator(sign_from + 1 + splitPoint, sign_to, decimal_from, decimal_to,
						fraction_from, fraction_to);
				...
				final ForkComparator f1 = new ForkComparator(sign_from, sign_to, decimal_from, decimal_from + splitPoint,
						fraction_from, fraction_to);
				f1.fork();

				final ForkComparator f2 = new ForkComparator(sign_from, sign_to, decimal_from + 1 + splitPoint, decimal_to,
						fraction_from, fraction_to);
